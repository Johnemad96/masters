...
def generate_folder_name(base_folder):
    """Generate a folder name in the format of "YYYYMMDD_x" where x is the next available sequence number"""
    ...
    for folder in folders:
        if folder.startswith(today):
            try:
                seq = int(folder.split("_")[-1])
                if seq > max_seq:
                    max_seq = seq
            except ValueError:
                pass
    next_seq = max_seq + 1
    folder_name = f"{today}_{next_seq}"
    return folder_name

def create_dataset_folder(dir_path=os.getcwd()):
    dir_path += '/dataset'
    ...
    for folder in os.listdir(dir_path):
        ...
        if folder.startswith(date_str):
            folder_seq_num = folder.split('_')[-1]
            if folder_seq_num.isdigit():
                seq_num = max(seq_num, int(folder_seq_num)+1)
    folder_name = f"{date_str}_{seq_num:d}"
...
filenameStr = create_dataset_folder() #"20230305_2"
designedPath = readPath()

client = carla.Client('127.0.0.1', 2000)
# world = client.reload_world()
# print(client.get_available_maps())
stereoBaseLine_Kitti = 0.54
stereoBaseLine_Euroc = 0.11
...
# spawn_point = waypoints[1018].asctec fireflytransform
# loop
# spawn_point = waypoints[2795].transform
# L-Shape
# spawn_point = waypoints[1601].transform
savePath(filenameStr,designedPath)
spawn_point = waypoints[designedPath.pop(0)].transform

# zigzag
# complex loop
# spawn_point = waypoints[1604].transform
# spawn_point = waypoints[1757].transform
# spawn_point = waypoints[1179].transform
# spawn_point = waypoints[1749].transform
# spawn_point = waypoints[684].transform
...
camera_init_trans_left = carla.Transform(carla.Location(z = 1.7, y=-stereoBaseLine_Euroc/2, x=0.5))
camera_init_trans_right = carla.Transform(carla.Location(z = 1.7, y=stereoBaseLine_Euroc/2, x=0.5))
...
#stddev acc
# imu_bp.set_attribute('noise_accel_stddev_x', '0.02')
# imu_bp.set_attribute('noise_accel_stddev_y', '0.02')
# imu_bp.set_attribute('noise_accel_stddev_z', '0.02')

# #stddev gyro
# imu_bp.set_attribute('noise_gyro_stddev_x', '0.0017')
# imu_bp.set_attribute('noise_gyro_stddev_y', '0.0017')
# imu_bp.set_attribute('noise_gyro_stddev_z', '0.0017')

# #biasgyro
# imu_bp.set_attribute('noise_gyro_bias_x', '0.000194')
# imu_bp.set_attribute('noise_gyro_bias_y', '0.000194')
# imu_bp.set_attribute('noise_gyro_bias_z', '0.000194')

# imu = world.spawn_actor(imu_bp,carla.Transform(carla.Location(x=0, y=0, z=0),carla.Rotation(roll=90, pitch=90, yaw=-90)), attach_to =vehicle,attachment_type=carla.AttachmentType.Rigid)
imu = world.spawn_actor(imu_bp,carla.Transform(carla.Location(x=0, y=0, z=0)), attach_to =vehicle,attachment_type=carla.AttachmentType.Rigid)
...
# trafficManager = client.get_trafficmanager(8000) 
...
# Complex loop
# path = [waypoints[791],waypoints[1726],waypoints[1628]]
# L-Shape
# path = [waypoints[1982],waypoints[1089],waypoints[1196],waypoints[2203],waypoints[1403],waypoints[2805],waypoints[1621]]
path = []
...
imuDelay = 1 * 100 #5 seconds in milliseconds
end_time = start_time + imuDelay 
current_time = 0
...
for p in path:
    ...
    while True:
        ...
        #adding 5 to 10 seconds delay for the IMU to initialize (asusming I understood the paper correctly)
        if current_time > end_time:
            vehicle.apply_control(ba.run_step())
        current_time = world.tick()
...
plt.savefig("dataset/"+filenameStr+"/Path.png")
...
while i < len(imu_data_list['timestamp']):
    ...
    data = pd.DataFrame([[imu_data_list['frame'][i], imu_data_list['timestamp'][i],
                        ...
                        imu_data_list['gyro'][i].x * -1, imu_data_list['gyro'][i].y*-1, imu_data_list['gyro'][i].z*-1, 
...
data = pd.DataFrame( columns=['frame', 'timestamp','x','y','z','qw','qx','qy','qz','v_x (m/s)','v_y (m/s)','v_z (m/s)','w_x (rad/s)','w_y (rad/s)','w_z (rad/s)','acc x (m/s^2)','acc y (m/s^2)','acc z (m/s^2)','roll','pitch','yaw'])
data.to_csv(csvfilename, index=False,header="auto",mode='a')
csvfile = open(csvfilename, "a")
i=0
while i < len(groundTruth['timestamp']):
    ...
    data = pd.DataFrame([[groundTruth['frame'][i], (int)((groundTruth['timestamp'][i])*(10**9)),
                        ...
                        np.radians(((groundTruth['transform'][i].rotation.pitch * -1))),
                        np.radians((groundTruth['transform'][i].rotation.yaw)*-1)]],
                        columns=['frame', 'timestamp', 'x', 'y', 'z', 'qw', 'qx', 'qy', 'qz', 'v_x (m/s)', 'v_y (m/s)', 'v_z (m/s)', 'w_x (rad/s)', 'w_y (rad/s)', 'w_z (rad/s)', 'acc x (m/s^2)', 'acc y (m/s^2)', 'acc z (m/s^2)','roll','pitch','yaw'])
    data.to_csv(csvfilename, index=False,header=False,mode='a')
    i+=1
csvfile.close()
print("GroundTruth data is saved in " + str(csvfilename) + " file with " + str(i) + " samples")
time.sleep(1)
