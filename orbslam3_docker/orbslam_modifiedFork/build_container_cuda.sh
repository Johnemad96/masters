...
docker rm -f orbslam3_modifiedFork &>/dev/null
[ -d "ORB_SLAM3" ] && sudo rm -rf ORB_SLAM3 && mkdir ORB_SLAM3

# Create a new container
docker run -td --privileged --net=host --ipc=host \
    --name="orbslam3_modifiedFork" \
    --gpus=all \
    -e "DISPLAY=$DISPLAY" \
...
# docker exec -it orbslam3_modifiedFork bash -i -c "git clone https://github.com/Johnemad96/ORB_SLAM3 /ORB_SLAM3 && cd /ORB_SLAM3 && chmod +x build.sh && ./build.sh "

# # Compile ORBSLAM3-ROS
# docker exec -it orbslam3_modifiedFork bash -i -c "echo 'ROS_PACKAGE_PATH=/opt/ros/melodic/share:/ORB_SLAM3/Examples/ROS'>>~/.bashrc && source ~/.bashrc && cd /ORB_SLAM3 && chmod +x build_ros.sh && ./build_ros.sh"
